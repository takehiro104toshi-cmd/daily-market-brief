...
| G10 | **TOPIX指数の供給元** | yfinanceに指数symbolなし（legacyの1306.TはETF——指数seriesへ流用しない）。Stooq ^tpxはG9で不達。JPX公式サイトに自動取得向け公開CSV/API確認できず（P2-G probe run #6） | **P2-G: J-Quants（JPX公式系API）採用・adapter実装済み・API到達実証**。**P2-G.1**: credential resolver契約（id_token/refresh_token/mail_password・差替可能）＋秘密safety（Secret型・scrub・非永続）＋identity guard（ETF NAV/先物拒否）＋freshness gating（当日利用可否をG10状態へ反映）。run #8はcredential未投入のため**TOPIX_CREDENTIAL_MISSING**で正常停止（認証リクエスト0回）。live取得はユーザーのJ-Quants登録＋repo secrets投入後に同pilotで自動実証。ETF代用はしない | **BLOCKED**（run #12実測。reason: `auth_failure` / `error:auth_error`。投入されたJQUANTS_API_KEYは5搬送方式すべてで403——現行APIの正式な認証方式であることを確認できず。有効なmail+password / refreshToken投入後、鮮度により RESOLVED または HISTORICAL_RESOLVED_CURRENT_BLOCKED へ機械遷移） |
