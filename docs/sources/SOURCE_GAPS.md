...
| G10 | **TOPIX指数の供給元** | yfinanceに指数symbolなし（legacyの1306.TはETF——指数seriesへ流用しない）。Stooq ^tpxはG9で不達。JPX公式サイトに自動取得向け公開CSV/API確認できず（P2-G probe run #6） | **P2-G.2: J-Quants V2 `/v2/indices/bars/daily/topix`（API Keyヘッダ認証）を採用。V1は2026-06-01終了のためrun #7〜#12の403は`legacy_v1_endpoint`/`api_version_mismatch`へ再分類（記録はappend-only保全）。run #14はLight未満で`plan_not_entitled`。Light plan投入後のrun #15で実証** | **RESOLVED**（run #15実測: HTTP 200・`auth_method_validated: api_key_header`・268営業日 2025-07-28〜2026-09-01・latest close 4181.86（as_of 15:30 JST）・freshness **CURRENT_USABLE**（gap_sessions 0）・Morning Compass利用可・MARKET_OBSERVATION QA accept 268件・canonical/SQLite再構築一致・NT倍率266行。ETF/先物代用なし） |
