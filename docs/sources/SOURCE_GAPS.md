...
| G10 | **TOPIX指数の供給元** | yfinanceに指数symbolなし（legacyの1306.TはETF——指数seriesへ流用しない）。Stooq ^tpxはG9で不達。JPX公式サイトに自動取得向け公開CSV/API確認できず（P2-G probe run #6） | **P2-G.2: J-Quants **V2**（`/v2/indices/bars/daily/topix`）を採用・provider実装済み・live到達実証。V1は2026-06-01終了のため、run #7〜#12の403は`legacy_v1_endpoint` / `api_version_mismatch`へ再分類（記録はappend-only保全）**。ETF代用はしない | **BLOCKED**（run #14実測。reason: `access_level_insufficient` / `plan_not_entitled`。V2応答は「This API is not available on your subscription」——**endpoint・API版数・API Keyの搬送方式は正しく、サーバは契約を識別している**。残る障害は**プラン権限のみ**で、TOPIX四本値はLightプラン以上が条件（公式docs＋live応答の2系統で一致）。プランをLight以上へ変更すれば、鮮度に応じ RESOLVED または HISTORICAL_RESOLVED_CURRENT_BLOCKED へ機械遷移する。コード側での迂回・代用はしない） |
