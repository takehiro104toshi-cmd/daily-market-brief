"""MarketWatchの公開見出しRSS（ベストエフォート）。

重要な注意（必ず読むこと）:
    MarketWatchの本文取得・転載は一切行わない。ここで参照するのは「公開
    されている見出し一覧RSS」のエンドポイントのみであり、この開発環境
    （サンドボックス・外部ネットワーク遮断）では実際の接続確認ができていない
    「ベストエフォート」実装である。本番運用前に、URLが現行の公開仕様と
...
MARKETWATCH_SOURCES = [
    ...
    {"name": "MarketWatch RealTime Headlines", "url": "https://feeds.marketwatch.com/marketwatch/realtimeheadlines/"},
]

RELIABILITY = {"MarketWatch Top Stories": 0.75, "MarketWatch RealTime Headlines": 0.7}


def fetch_marketwatch_headlines(
    sources: SourceRegistry, limit: int = 8, news_sources: Optional[list] = None
) -> List[Headline]:
    return fetch_headlines(news_sources if news_sources is not None else MARKETWATCH_SOURCES, sources, limit, reliability_map=RELIABILITY)
