...
## v3.5 Market Narrative & Section Pruning（本日の相場総括・構成整理）

情報を並べるだけでなく「今日の相場がなぜ動いたのか」を端的に読める構成にしました。

### 📝 本日の相場総括（Market Narrative）

HTML最上部（Today's Decision・Dashboardより上）に新設した総括カードです。
市場データ・重要ニュースと、既に算出済みの各分析エンジン（Market Regime・
Cross Market・Future Intelligence・今週の重要イベント・異常値検知・Analysis
Confidence）だけを**機械的に組み合わせて**、次を端的にまとめます。

- 今日を一言で（例:「米金利上昇と半導体株安を背景に、リスクオフ寄りの展開」）
- 何が起きたか（日経・米国株・SOX・VIX・ドル円・金利・原油・金の主要変化）
- なぜ動いたか（Cross Marketの波及＋テーマと株価の乖離＋地合いへの寄与）
- 背景／波及チェーン／これから見るべき点（金利・SOX・ドル円・イベント）
- 今後の見立て（**断定せず条件分岐**で記載）／注意すべきリスク
- 投資判断への示唆（短期/中期/長期/注意。**売買助言はしません**）

生成AIの作文・断定的な将来予測・個別の売買推奨は行いません（ルールベース）。

### 役割の違い

- **本日の相場総括**: 「なぜそうなったか」の深掘り総括。
- **Today's Decision**: 「3分で見る判断カード」（短い要約）。
- 重複しないよう、詳しい説明は総括側に集約しています。

### 構成の整理

- シナリオ系は「今日の3大シナリオ（期待値順）」を主軸にし、「個別シナリオ」は
  重複を避けて折りたたみ表示にしました。
- 営業支援系（今日電話すべき顧客／営業準備／営業トーク／各種営業コメント／朝会
  コメント／会話ネタ／想定質問）は「営業メモ」として1グループに集約し、各カードは
  初期折りたたみ（表示オプションの「営業セクションを非表示」で一括で隠せます）。
- Future Intelligence・Watchlist・引用は、既存版（v2.7/v3.1）で導入済みの
  「要約＋詳しく折りたたみ」を継続しています。
