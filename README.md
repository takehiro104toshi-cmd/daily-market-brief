...
1. **ルールベースの機械的分析エンジン（常時有効・追加設定不要）**
   ...
   外部APIも追加費用も不要で、この層だけで全29セクションが完成します。
...
営業一言を200〜300文字程度に凝縮）を表示したのち、以下29セクションが続きます。

1. AI Executive Summary（AI分析・今日最重要ニュース最大3件。結論／理由／日本株への影響／
   ドル円への影響／金利への影響／営業トークを1件ずつ表示）★★★★★
2. 今日の結論（3行・太字）★★★★★
3. 今日の注目5銘柄（AI分析・日本株5銘柄・米国株5銘柄、コード／企業名／理由／注目材料／短期見通し）
4. 主要指標（事実）
5. 為替・金利（事実）
6. 今日の相場シナリオ（AI分析・強気／中立／弱気それぞれの確率・理由・注目指標）★★★★★
7. 日経平均・ドル円・米国市場 個別シナリオ（AIシナリオ・指標ごとの強気／中立／弱気3シナリオ）
8. 今日の重要ニュースランキング（AI分析・理由／影響市場／影響業種／営業トークつき）
9. マーケットインパクト（AI分析・日経平均／TOPIX／ドル円／長期金利／半導体／銀行／商社／
   自動車／海運／電力／素材／不動産の12対象への影響度★・プラス/マイナス/中立の判定一覧）
10. セクターランキング（AI分析・本日「強そう／弱そう」の予測を↑/→/↓と理由で表示）
11. 今日見るべき指標（為替・VIX・米10年・WTI・Gold の節目ラインと一言コメント）
12. マーケット分析（AI分析・因果関係を矢印↓で整理／個別の因果チェーン3〜5本）
13. テーマ分析（AI分析・ランキング形式、今後1週間／1か月／3か月の見立て）
14. 業界ランキング TOP10（AI分析・追い風／逆風／関連銘柄／営業トーク）
15. 個別株ランキング（AI分析・日本株TOP10／米国株TOP10、短期／中期／長期）
16. 今日のウォッチリスト（★評価＋1行理由の一覧）
17. 保有・監視銘柄分析（AI分析・全銘柄について今日／1週間／1か月／長期／リスク）
18. 長期投資アイデア TOP5（AI分析）
19. 今日電話すべき顧客（AI分析・富裕層／NISA／退職金／法人／相続／若年層の6顧客タイプ別、
    理由／話題／営業トーク例）
20. 営業準備（社長向け一言・富裕層向け話題・初心者向け用語解説・今日の雑談・想定質問）
21. 営業トーク（法人社長向け／個人投資家向け／初心者向け／富裕層向け）
22. 営業向けコメント（AI分析・法人社長／富裕層／個人投資家／NISA初心者／為替関心／
    米国株関心／日本株関心の7オーディエンス別、各約30秒で話せる長さ）
23. 想定質問と回答例（AI分析・日経平均／円安／NVIDIA／注目業種／NISAの5問を含む拡張Q&A）
24. 岡三証券営業向けコメント（AI分析・富裕層／法人／NISA／退職金／相続の5顧客タイプ別、
    各約30秒で話せる長さ。セクション名は例であり、`sections.render_okasan_sales_comments`
    と `builder.py` の見出し文字列を書き換えれば任意の支店・チーム名に変更できます）
25. 朝会コメント（AI分析・30秒／1分／3分の3パターン、朝会でそのまま読み上げられる自然な文章）
26. 今日の会話ネタ（AI分析・3つ）
27. イベント（事実・今日／今週／今月）
28. AIまとめ（AI分析・300文字以内の戦略サマリー）
29. 引用（事実・参照URL一覧）

- 見出し・銘柄には重要度・変化率に応じた★評価を表示します。
- データを取得できなかった項目は空欄にせず「取得不可」と明記し、
  29セクションの構成自体は常に維持されます（一部データが欠けてもレポートは崩れません）。
...
- [ ] **output生成の確認**
  ...
  5つが生成されていることを確認してください。詳細版は29セクションすべてが存在し、
...
3. もっと詳しく知りたい場合は `output/latest_market_brief.md`
   （詳細版・全29セクション）を開いてください。詳細版の冒頭にも
...
daily-market-brief/
  ...
  src/
    ...
    analysis/               # 「AI分析」を担うルールベースの考察エンジン一式
      ...
      executive_summary.py  # AI Executive Summary（今日最重要ニュース最大3件、日本株/ドル円/金利への影響）
    ...
    report/
      ...
      builder.py             # 29セクションのMarkdownレポート組み立て（詳細版・組み立て役のみ）
