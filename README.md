...
### 6. 手動で最新情報に更新する（「最新情報に更新」ボタン）

毎朝7:00の自動更新を待たずに、必要なタイミングで最新ニュース・最新マーケット
データにレポートを再生成したい場合は、HTML版（Today's Dashboardのすぐ下）に
ある **「🔄 最新情報に更新」ボタン** を使います。

**使い方:**

1. スマホ（またはPC）でGitHub Pagesのレポートページを開く。
2. 「🔄 最新情報に更新」ボタンをタップする。
3. 遷移した先のGitHub Actions「Daily Market Brief」ワークフローページで、
   右上の **「Run workflow」** ボタンを押す（ブランチはデフォルトのままでOK）。
4. 数分待ってから、GitHub Pagesのレポートページを再読み込みする
   （ワークフローが完了すると自動的に再デプロイされます）。

このボタンは、GitHub Actionsの実行ページへ遷移するだけの安全なリンクです
（ワンクリックでは自動実行されません。GitHubへのログイン・Run workflowの
操作はユーザー自身が行います）。ボタンのリンク先は、環境変数
`GITHUB_REPOSITORY`（GitHub Actions実行時に自動設定）から自動的に組み立てられ、
`config.yaml` の `output.actions_url` または環境変数 `ACTIONS_URL` で
上書きできます。いずれも未設定の場合（ローカル実行時など）はボタン自体が
表示されません。

**次段階の案（今回は未実装）:** GitHub Personal Access Token（PAT）を
Secretsに登録し、GitHub APIの `workflow_dispatch` エンドポイントを
JavaScriptから直接呼び出せば、ボタンを押すだけで完全自動のワンクリック更新も
可能です。ただしPATの発行・Secrets管理・トークン漏洩リスクなど認証面の
考慮が必要なため、まずはユーザー自身がGitHubにログインしてRun workflowを
押す、より安全なリンク方式を採用しています。
